"""M15_ccv - Perfil Clutch del Jugador (ensamblaje scout-facing final).

Combina outputs de M14 (CATE jerarquico bayesiano con eta_ga, eta_gf,
eta_pressure) + M04/M06/M08-M13 + metadata para producir outputs/ccv_table.parquet.

3 dimensiones del CCV:
...
Outputs:
    outputs/ccv_table.parquet                                 1 fila/jugador (~299 cols)
    outputs/ccv_aux/top10_{chasing,protecting,pressure}_per_position.parquet
    outputs/ccv_aux/top10_{chasing,protecting,pressure}_per_bucket.parquet
    outputs/ccv_aux/dual_clutch_top.parquet
    outputs/ccv_aux/by_team.parquet

Uso:
    python M15_ccv.py [overwrite]
"""
...
# ---- Schema contract estable de la tabla CCV ----
# Campos requeridos en ccv_table.parquet. Si falta alguno, M15 falla con
# mensaje claro antes de persistir — protege a downstream consumers
# (notebooks scout, exports, etc.) de schemas rotos.
CCV_REQUIRED_COLS: dict[str, type] = {
    # Identidad (6)
    ...
    # CCV 4-vec summary (4)
    "ccv_atk": pl.Float64, "ccv_def": pl.Float64,
    "ccv_off": pl.Float64, "ccv_phys": pl.Float64,
...
def validate_ccv_schema(df: pl.DataFrame) -> None:
    """Falla con mensaje claro si faltan cols requeridas o tipos no coinciden.

    Garantia de estabilidad del contrato ccv_table.parquet. Llamada
    obligatoria antes de write_parquet en main().
    """
    missing = [c for c in CCV_REQUIRED_COLS if c not in df.columns]
    if missing:
        raise ValueError(f"CCV schema FALTAN cols: {missing}")
    bad_types = []
    for c, expected in CCV_REQUIRED_COLS.items():
        actual = df[c].dtype
    ...
    if bad_types:
        raise TypeError(f"CCV schema TYPES wrong: {bad_types}")
...
def _load_physical_per90() -> pl.DataFrame:
    ...
    La normalizacion per-90 se hace en build_ccv_table tras join con minutes.
    """
...
def _load_baseline_channels() -> pl.DataFrame:
    ...
    M08 score_atk, M09 score_def). Permite distinguir 'ccv_off bajo porque baseline
    elite ya' vs 'ccv_off bajo porque mediocre'.
    """
    rows = []
...
# ---- Vector CCV summary 4-canal directional ----

def _build_ccv_summary_vector(cate_wide: pl.DataFrame) -> pl.DataFrame:
    """4-vector directional: cada canal usa shock_type de máxima leverage.

    ccv_atk  = cate_ataque_GOAL_AGAINST_mean  (chasing)
    ccv_def  = cate_defensa_GOAL_FOR_mean    (protecting)
    ccv_off  = cate_offball_GOAL_AGAINST_mean (chasing)
    ccv_phys = cate_fisico con max-magnitude signed (GA o GF, el más reactivo)
    """
    return cate_wide.with_columns([
        pl.col("cate_ataque_GOAL_AGAINST_mean").alias("ccv_atk"),
        pl.col("cate_defensa_GOAL_FOR_mean").alias("ccv_def"),
        pl.col("cate_offball_GOAL_AGAINST_mean").alias("ccv_off"),
...
def build_ccv_table() -> pl.DataFrame:
    ...
    print("[M15] Vector CCV summary 4-canal directional...")
    cate_wide = _build_ccv_summary_vector(cate_wide)
    ...
    front = ["pff_player_id", "player_name", "team_name", "position_group",
             ...
             "ccv_atk", "ccv_def", "ccv_off", "ccv_phys",
...
def build_aux_tables(ccv: pl.DataFrame) -> dict:
    """Tablas auxiliares scout-friendly."""
    aux = {}
    # Top10 chasing per position
    aux["top10_chasing_per_position"] = (ccv.sort("chasing_clutch_idx", descending=True)
    ...
    aux["top10_protecting_per_position"] = (ccv.sort("protecting_clutch_idx", descending=True)
        .group_by("position_group", maintain_order=True).head(10)
    ...
    outfield = ccv.filter(pl.col("position_bucket") != "GK")
    ...
    if "pressure_response_idx" in ccv.columns:
        aux["top10_pressure_per_position"] = (ccv
    ...
    by_team = (ccv.group_by("team_name").agg([
...
def main():
    ccv = build_ccv_table()
    validate_ccv_schema(ccv)             # falla si schema contract roto
    _OUT_DIR.mkdir(parents=True, exist_ok=True)
    _AUX_DIR.mkdir(parents=True, exist_ok=True)
    out_path = _OUT_DIR / "ccv_table.parquet"
    ccv.write_parquet(out_path)
    print(f"\n[M15] Saved {out_path} ({ccv.height} jugadores, {ccv.width} cols)")

    aux = build_aux_tables(ccv)
    ...
    # Resumen sanity
    print(f"\n[CCV Table summary]")
    n_low = int(ccv["low_sample"].sum())
    print(f"Jugadores n_shocks>={MIN_SHOCKS}: {ccv.height} "
           f"({n_low} con low_sample flag)")
    print(f"Posiciones cubiertas: {ccv['position_group'].n_unique()}")
    print(f"Equipos cubiertos: {ccv['team_name'].n_unique()}")
    print(f"\nDistribucion sig_chasing:")
    print(ccv.group_by("sig_chasing").len().sort("len", descending=True))
    print(f"\nDistribucion sig_protecting:")
    print(ccv.group_by("sig_protecting").len().sort("len", descending=True))
    print(f"\nTop 10 Remontador globales:")
    print(ccv.sort("rank_chasing_global").head(10).select(
    ...
    print(ccv.sort("rank_protecting_global").head(10).select(
